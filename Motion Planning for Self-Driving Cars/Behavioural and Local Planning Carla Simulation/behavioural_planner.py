...
class BehaviouralPlanner:
    def __init__(self, lookahead, stop_sign_fences, lead_vehicle_lookahead):
        self._lookahead = lookahead
        self._stop_sign_fences = stop_sign_fences
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        # ------------------------------------------------------------------
        waypoints_ = np.array(waypoints)
        while wp_index < len(waypoints) - 1 and arc_length < self._lookahead:
            wp_index += 1
            arc_length += np.linalg.norm(waypoints_[wp_index][0:2] - waypoints_[wp_index - 1][0:2])
        # ------------------------------------------------------------------
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        ...
        for i in range(closest_index, goal_index):
            ...
            for stop_sign_fence in self._stop_sign_fences:
                wp_1 = np.array(waypoints[i][0:2])
                wp_2 = np.array(waypoints[i + 1][0:2])
                s_1 = np.array(stop_sign_fence[0:2])
                s_2 = np.array(stop_sign_fence[2:4])
